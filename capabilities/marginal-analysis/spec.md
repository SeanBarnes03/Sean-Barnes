...
`MAX_BEDS + 1`, and `TOTAL_COST(0)` is zero.  *Added "+1" max beds as a diagnostic only.  
...
## 3.13 Shadow Price 
Profit at the optimum with that crop's cap +1, minus profit at the optimum.
---
...
- "PASS" means the check cell displays the text PASS. This spec uses PASS and FAIL
  ...
  cross-sheet links.
- For each capped crop, PRICE − MC(cap+1) must equal the §3.13 shadow price, within your existing tolerance.
